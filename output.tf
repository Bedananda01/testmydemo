                  ...
                  inputs:
                    ...
                    inlineScript: |
                      ...
                      chmod -R +x .terraform/providers/ 2>/dev/null || true
                        ...
                        # Also check role assignment
                        if grep -q "RoleAssignmentExists" /tmp/tf_apply_output.txt; then
                          ...
                          if [ -n "$EXISTING_ROLE" ] && [ "$EXISTING_ROLE" != "None" ]; then
                            terraform import -var-file=terraform.tfvars \
                              ...
                              "$EXISTING_ROLE" 2>&1 && IMPORTED=$((IMPORTED+1)) || true
                          fi
                        fi

                          echo ""
                          echo "══════════════════════════════════════════════"
                          echo "  TERRAFORM APPLY (attempt 2 — after import)"
                          echo "══════════════════════════════════════════════"
